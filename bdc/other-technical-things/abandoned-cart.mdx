...
Explain DB and APIs used + flow. Feel free to refer to docs we already have explainaing stripe_checkout_customer. If there's anything lacking in those docs, please fill in gaps here
...
- **Persistent cart record (database)**:\
  ...
  For BDC this record includes : `createdOn`, `levelId`, `locationId` / `location`, and `firstName` / `studentName` / `lastName`.
