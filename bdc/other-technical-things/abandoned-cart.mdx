...
- **Persistent cart record (database)**:\
  When a user reaches checkout with a program in their cart, the backend saves a **checkout record** for that user (keyed by `memberId`).\
  The record includes (at minimum): `createdOn`, `programStartDate` / `programEndDate` (NSD), `levelId` or `slug`, `locationId` / `location`, `firstName` / `studentName` / `lastName`, and `programCategoryId` (to distinguish residential/commuter/online).
- **Browser storage (localStorage)**:\
  The frontend mirrors the last checkout record in `localStorage` under the key `checkOutData` so it can show abandoned-cart UI without hitting the API every time.\
  It also stores `isAbandonedModalOpen` and `lastModalClosedDate` to enforce the “show at most once every 7 days” rule.

### APIs

- **Get abandoned cart by member**
  - Endpoint (BDC/NSD): `GET {baseUrl}/getCheckoutUrlByMemberId/{memberId}` or `GET {baseUrl}/getCheckoutURLByMemberId/{memberId}`
  - Used when `checkOutData` is missing from `localStorage`.
  - Returns a JSON object with the fields listed above (`createdOn`, program dates, label/level, location, student name, slug, `programCategoryId`, etc.).
...
1. **Initialization**
   ...
   - It receives config such as `baseUrl`, `memberId`, `modalId`, and `hour` (minimum age in hours before we consider a cart “abandoned”).
2. **Load cart data**
...
3. **Eligibility checks before showing anything**\
   ...
   - For NSD, `programStartDate` is **strictly in the future**; if the program has already started, no reminder is shown.
   ...
   - For BDC, there is an additional check that the current selling session has started; if not, stale `checkOutData` is cleared.
...
5. **Show abandoned-cart modal**
   - If all checks pass, the script:
     ...
     - Builds the **“View Cart / Complete Purchase”** link by combining `window.location.origin` with the program identifier from `checkOutData`:
       - BDC: uses `levelId` to create a `/programs/level-{levelId}?returnType=back` URL.
       - NSD: uses `slug` and `programCategoryId` to route to the correct `cart/{slug}` URL (residential/commuter/online) with `returnType=back`.
