...
- **Persistent cart record (database)**:\
  When a user reaches checkout with a program in their cart, the backend saves a **checkout record** for that user (keyed by `memberId`) in the `stripe_checkout_customer` collection. This record includes : `createdOn`, `levelId`, `locationId` / `location`, and `firstName` / `studentName` / `lastName`.
