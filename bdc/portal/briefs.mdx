...
[Managing Topics](https://bergendebate.retool.com/apps/b98e67d2-9528-11f0-a770-eb637de74341/Q3%6025/Manage%20Briefs/page1)  page details are documented in the **Briefs Management** section. Please refer to the doc link - bdc/checkout-systems/briefs/management

## Technical

<Note>
  TODO: Reorganize as follows

  1. DB (needs to be added) -- at least whatever is not already covered in "Briefs Checkout" page
  2. Flow
     1. Portal visibility
     2. Portal render
</Note>
...
The portal "My Briefs" page shows only briefs the member has **already purchased**. Data comes from **getTopicsDetails** (with **memberId**), which reads **topic_payment_details** (purchases per member) and **topic_details** (catalog: name, order, version, PDF URLs). Full schema for **topic_payment_details** and post-purchase writes are covered in [Briefs Checkout](/bdc/checkout-systems/upsells/briefs). Below is what the **portal** uses.
...
1. **JS (BriefManager)** — init and request
   - File: **portal-briefs.js**; class: **BriefManager**. Parent page must pass **baseUrl** and **memberId**: `new BriefManager({ baseUrl: '...', memberId: '...' });`.
...
2. **API (getTopicsDetails.py)**
   ...
   - (Without memberId the API returns the full catalog; used for checkout—see [Briefs upsells](/bdc/checkout-systems/upsells/briefs).)
