...
Portal Invoices (Payment History page) shows per-student outstanding invoices and completed enrollments—one main JS file (**payment_history.js**) and **getPortalDetail** (member-service). 
...
- The portal does **not** call a dedicated invoice-only API. **getInvoiceList** (via **getInvoiceListScript**, e.g. **memberScript.py** / **invoiceScript.py**) runs aggregation on **invoice_notifications** (match **paymentId**), then joins **invoice_records**, **invoice_payment_link**, **invoice_responses**, **stripe_checkout_customer**.
...
| **classDetail** / **summerProgramDetail** | Optional; used for session label and **generateItemizedInvoice** by **yearId**. |
...
| **is_completed**   | `true` if at least one **invoice_response** exists for this invoice + paymentId; else unpaid/pending.                                                                                                                                                    |
...
| **jotFormUrlLink** | Array of payment options: **paymentLinkId**, **title**, **amount**, **paymentType** (e.g. `card`, `us_bank_account`), **jotFormUrl**, **formid**. Used to render Pay / JotForm links.                                                                    |
...
The portal uses **currentSession** and **futureSession** to build Outstanding Invoices (invoices where **!invoice.is_completed**), and **currentSession** + **pastSession** to build Completed Enrollments / Payment history (by **currentYear**) and to call **generateItemizedInvoice** by **yearId**.
...
- **Main content** depends on **getPortalDetail** response:
  - **Has portal data:** Show the Payment History UI (student tabs, Outstanding Invoices, Completed Enrollments / Payment history). First student is selected by default; `.portal-info-wrapper` and `.portal-tab` shown after **Webflow.require("tabs").redraw()**.
...
3. **JS (PaymentHistory)** — after response
   ...
   - **Invoice Stripe pay:** **initializeStripePayment** — POST **createCheckoutUrl** (amount in cents, invoiceId, paymentId, etc.); on success redirect to Stripe.