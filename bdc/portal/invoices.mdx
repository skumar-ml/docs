...
- **Completed Enrollments (usable for tax purposes)**
  ...
  <Frame>
    ![Sample PDF receipt from Completed Enrollments](/images/bdc_portal_receipt_sample.png)
  </Frame>
...
##### **Planned PDF receipt fields (Lambda)**

The portal’s **Download Receipt** flow calls `generateItemizedInvoice` (see [Non-Technical](#non-technical) sample). Backlog for the PDF generator:

- **Services rendered** — Add a line: **semester** purchases → `Semester debate class`; **summer** purchases → `Summer debate camp`.
- **Dates of service** — Add a line with the program start and end dates from the DB-backed session or enrollment record used when resolving the purchase (step 3 above).
- **Deposit in total** — Add the paid **deposit** to the receipt **Total**, not only as a footnote. **Line items and totals match the portal Invoice Breakdown.**
