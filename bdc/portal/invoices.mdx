...
- A frontend request (via API Gateway)
- User clicks **“Download Receipt”** for a student or family

**What it figures out internally**

1. **Identifies the family**
   - Starts from a parent `memberId`
   - Finds all related family members
   - Ensures receipts include applicable student and payments
2. **Collects all relevant payments**
   - Looks across multiple payment types:
     - Academic classes
     - Summer programs
     - Topic add-ons / bundled payments
...
4. **Applies pricing rules**
   ...
   - Applies:
     - Discounts (early bird, sibling, promo codes)
     - Credits and adjustments
   - Handles:
     - Deposits
     - Bucket vs non-bucket invoices
   - Ensures totals never go negative
5. **Builds receipt data**
   - For each purchase, it prepares:
     - Student name
     - Program names
   ...
   - This data is **structured**, not just formatted text.
...
7. **Stores and returns the receipt**
   ...
   - Returns a public URL to the frontend
