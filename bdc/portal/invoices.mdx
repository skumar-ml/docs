...
The invoice system flows from **Retool (admin)** creating and linking invoice data, through **member-service** aggregating it per student/session, to the **portal** rendering an accordion and handling Stripe payment. After payment, **scheduler_service** (Stripe webhook) writes **invoice_responses** and updates credits. Separate APIs support **payment history / itemized receipts** 
