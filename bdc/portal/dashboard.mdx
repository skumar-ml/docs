...
<Tabs>
  <Tab title="Student tabs">
    ...
    - **Render:** `setupTabs()` contains creates tab link + pane per student, then `renderStudentTab() `per pane
  </Tab>
  ...
  <Tab title="Registration Forms header">
    ...
    - **Render:** `renderInvoiceAccordionStyled()`, `renderRegistrationForms()`; 
  </Tab>
  <Tab title="Calendar">
    <img
  ...
  <Tab title="Next Recommended Program">
    See the returner conversion page for details.
