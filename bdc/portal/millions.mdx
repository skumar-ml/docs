...
Millions are **per-student** “earn” amounts (e.g. points or rewards) tied to the current academic year/session\<@**DEV: need to confirm**\>. Data is stored in **millions_transactions** (keyed by **studentId**). Admins or coaches update amounts via **POST updatePortalMillions** (member-service) or via Retool. The **portal** and **payment history** pages fetch **GET getMillionsTransactionData/** (member-service), which joins **payment_details** (current year/session)\<@**DEV: need to confirm**\> with **millions_transactions** by **studentId** and returns one entry per student with **earnAmount** and **transactions**. The portal (**portal_new_ui.js**) displays the millions count in the sidebar (**[data-millions="sidebarCount"]**) as “XM” for the active student tab. 
...
- **GET getMillionsTransactionData/** (member-service, **getMillionsTransactionData.py**).
  ...
  - **currentYear**, **sessionId** = **getCurrentYearAndSessionId()**. Get **class_details** for that year/session; build **classIdDict** (list of **classUniqueId**).\<@**DEV: need to confirm**\>
...
- **POST updatePortalMillions** (member-service, **updatePortalMillions.py**).
  ...
  - Returns . Used by coach UI to add or update a student’s millions balance.
...
- **JS:** **portal_new_ui.js** and **payment_history.js** (**PaymentHistory** class). Both fetch **getMillionsTransactionData/** and pass **millions_transactions** into tab setup.
