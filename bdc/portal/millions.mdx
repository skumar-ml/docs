...
- **MillionsRenderer** runs only on the Millions page. It starts with the API URL and member ID, then loads the Millions transaction data.
- While loading, it shows a spinner and hides the page content. If no transactions are found, it shows a “no records” message and stops.
- If data exists, it displays the page, creates a tab for each student, and shows their balance and transaction history. Transactions are sorted from newest to oldest, and amounts are shown in green for earnings and red for deductions.
- When a student tab is selected, their total Millions amount is shown in the sidebar. The sidebar updates automatically whenever the user switches students.