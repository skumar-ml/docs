...
- All associated **student names** are displayed as selectable cards. The user can click on any student to view that student’s **Millions activity and balance**.
...
- The **Transactions table** displays the history of Millions activity for the selected student.Each entry shows the transaction **date, description,** and the **amount** added or deducted (± Millions).
...
Portal Millions shows **per-student** reward balances and transaction history for the [Millions page](https://www.bergendebate.com/portal/millions)—one JS file, one API. The page uses `millions_transactions` only;

**JS**

- File: `millions.js`; class: `MillionsRenderer`.
- Parent page must pass `apiBaseURL` and `memberId`: `new MillionsRenderer({ apiBaseURL: '...', memberId: '...' });`

**API**

- `GET getMillionsTransactionData/{memberId}` (**getMillionsTransactionData.py**).
- Resolves `memberId` to family member IDs; uses current year/session to find students with a payment in that term; for each student, looks up `millions_transactions` by `studentId` and returns `studentName, studentGrade, earnAmount, transactions`.
- Response includes **millions_transactions** (array per student); **items** is for the store (skipped).
...
**millions_items_details** — Catalog for the Millions store (skipped for this page).
...
<Note>
  TODO: Let's re-organize the technical section as follows:

  1. Describe DB collections (I think current description is good)
  2. Describe flow for millions page
  3. Describe sidebar & main content

  Basically, we can delete the current content above the DB, and combine it with the flow.
</Note>