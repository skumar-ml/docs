...
- **DOM:** **#half-circle-spinner**, **.portal-info-wrapper**, **[data-container="no-record-found"]**, **.portal-tab**, **.portal-tab-menus**, **.portal-tab-content**, **.w-tab-pane**, **.million-price-text**, **.transactions-table-div**, **[data-millions="sidebarCount"]**.

### Summary

The Millions page is powered by **millions.js** (**MillionsRenderer**). It fetches **getMillionsTransactionData/**, then renders one tab per student with balance and transaction history and updates the sidebar count. Data comes from **millions_transactions** (per **studentId**). The Millions store and **items** are out of scope.