...
## Registration & Checkout Flow (Overview)

### Step 1: Create Account

- Parent creates an account by entering basic details (name, email, phone, password).
- Existing users can log in using the **Login** button.

### Step 2: Student Details

- User can select an existing saved student profile or add a new student and the Order Details section with required details section is shown on the right side.

### Step 3: Select Class & Bundles

- User selects **location** and **class time**.
- Availability status is shown for each time slot (available / almost full / full).
- Optional future session bundles (**Winter/Spring or Summer**) can be selected.
  - When a future session is selected, it appears in the **Order Details (top-right)** with updated pricing and total.
- **Bergen Primers** (Logical Fallacies, Cognitive Biases, Argumentation) can be added.
  - Selecting a primer updates the Order Details with its price and revised total.

### Step 4: Payment

- User selects a payment method:
  - **Bank Transfer** (includes discount)
  - **Credit Card**
- On clicking **Pay Now** with Credit Card, the user is redirected to the **Stripe payment page**.
