...
<Note>
  TODO: KS, this is incorrect for the portal upsell. Portal upsells are managed [via this Retool page](https://bergendebate.retool.com/apps/84c8d61e-50a3-11f0-bd6f-77e684555d46/2025%20Phase%20B/upsell%20Program%20Mapping/page1). \
  \
  This section is good, but needs to go on the Dashboard page (for how admin manage portal content)
</Note>
...
3. **JS (DisplaySuppProgram)** — after response
   ...
   - **Academic-only:** The API returns programs for both the current session (`portal_program_ids`) and summer (sessionId 2). The frontend keeps only items where `sessionId !== 2`, so Summer is excluded and only academic-year sessions (e.g. Fall, Winter/Spring) are used.
     ...
     <Note>
       TODO: Yogesh, did we confirm the academic-only filtering is correct? I remember us offering summer upsells..

       **Comment**: We are displaying the selected upsell program in the portal using the portal_program_ids from the current session. Both summer and academic programs can be added to the portal_program_ids. We are displaying the selected upsell program in the portal using the portal_program_ids from the current session. Both summer and academic programs can be added to the portal_program_ids. Please refer to the image below
     </Note>
