...
- **POST addReferralData** (referral-promotion-service, **addReferralData.py**).
  ...
  - If **members** has existing member with that **email** and **familyId** set → 422 "Member already exists".
  - If **referral_details** has doc with that **email** → 422 "Member already referred".
  - **guestId:** if **members** has guest by **email**, use that **\_id**; else call **createAccount** API (parent sign-up, source "referral") and use returned **parent_id**.
  - **members.find_one()** → **familyId**. **coupon_details.find_one()** → **couponId**. Insert **referral_details**: **memberId**, **referral_familyId**, **guestId**, **date_referred** (now), **purchased_date** (None), **referred_stage** ("None"), **couponId**, **email_sent** (1). Create lead in CRM; send email (immediately_after_referral) with **coupon_code** and referrer name; schedule follow-up emails (e.g. 24h, 72h, 240h). Return 200.
