...
When pre-registration is active, users see a banner highlighting that registration is open, along with the remaining time to secure a spot.

- Clicking **Register Now** takes the user directly to the **Stripe payment page** to complete enrollment.
- Clicking **Learn More** opens a **Next Recommended Program** popup, showing details such as the recommended level, term, and program information.
...
Once pre-registration ends, the banner updates to reflect the new registration state.\
The countdown is no longer shown, and users are guided through the standard registration flow based on availability.
