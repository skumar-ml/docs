...
**API (**`getPortalDetail/{memberId}`)

- For each student in the family & the **current** session, the API returns **`recommendedLevel`** from `getRecommended(row)`
...
- File: **`portal_new_ui.js`**  class: **Portal**.
