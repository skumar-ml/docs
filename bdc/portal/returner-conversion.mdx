...
3. **JS (Portal)** — after response 3. Display logic uses two time windows: **pre-registration** (from `futureSession`) and **registration** (from `recommendedLevel`). Below, we detail when each section and button appears.
