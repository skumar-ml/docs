...
- **Next Recommended Program section** — Shown when the API returns **recommendedLevel** for that student (recommendation published). Displays next level and semester with **Register Now** and **Learn More**. 
- **Pre-registration block** — Shown when the student has a future-session bundle (no classId) and the current date is within the pre-registration window. Shows pre-reg status and countdown;
- **Main content** depends on API response and time windows: if **recommendedLevel** is present and the user is in the registration window, the Next Recommended Program section is visible; if the user has a bundle and is in the pre-reg window, the pre-reg block is visible; otherwise, the section can be hidden.