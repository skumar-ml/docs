...
- For each student in the family & the **current** session, if the student's recommendation (in the `members` collection) has `publish==True`, the API returns `recommendedLevel` 
