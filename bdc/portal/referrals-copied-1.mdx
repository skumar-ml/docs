...
<Note>
  TODO: Let's use the following format --

  1. Expain db and important fields
  2. Split next part into flows..
     1. Portal
        1. Rendering Portal Referrals Page
        2. Referrer submits referee details via form
        3. Referral visibility in portal sidebar
     1. Referree lands on website via referral link
</Note>
...
##### 1.1 Rendering Portal Referrals Page
...
- **loadReferralData()** → **GET getReferralData**. API: resolve member's `familyId`; if none, return. Get `coupon_code` and `couponId` from `coupon_details`; if no coupon, return `{}`. Read `referral_details` for this family; for each referral return `name`, `email`, `date_referred`, `purchased_date`, `referred_stage`. Response: `{ referrals, coupon_code }`.
...
##### 1.2 Referrer submits referee details via form
...
- **API (addReferralData):**
  ...
  - If email was already referred by the same referee `memberId` → reject; show **"Member already referred"**.
  ...
  - Find referee's `familyId` and their coupon. If no coupon → reject; show **"Coupon not found"**.
...
##### 1.3 Referral visibility in portal sidebar

- **JS:** `sidebar.js` calls `checkReferralsAccess(data)` after `setupTabs`.
- If any student has `currentSession` (length \> 0), set all `[sidebar-menu="referrals"]` to `display: flex`; otherwise `display: none`.

#### 2. lands on website via referral link

- **URL:** `https://www.bergendebate.com?code=...&id=...`
- **JS:** `referrals_modal.js`, `ReferralModal` class. On load, read `code` and `id` from URL; decode, store in `localStorage`, update discount code on page, fetch referrer name (e.g. `getMemberDetails`) for the modal.
- If the user has not yet submitted name and email, show the referral modal (discount code, referrer name, claim form).
- **Form submit** → **POST addReferralData**. On success: referral saved, confirmation shown, modal closes. On failure or duplicate: error message shown.
