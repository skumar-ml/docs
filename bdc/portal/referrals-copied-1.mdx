...
- **JS:** `referrals_modal.js`, `ReferralModal` class. On load, read `code` and `id` from URL; decode, store in `localStorage`, showing coupon code on page (TODO: via which API? @SK, Showing coupon code in the page. updated the description as well), fetch referrer name (`getMemberDetails`) for the modal.
