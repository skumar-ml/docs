...
- **JS:** `referrals_modal.js`, `ReferralModal` class. On load, read `code` and `id` from URL; decode, store in `localStorage`, update discount code on page (TODO: via which API?), fetch referrer name (`getMemberDetails`) for the modal.
