...
<Note>
  ...
  2. Split next part into flows..
     ...
     2. Referree lands on website via referral link
</Note>

### Overview

The referral system lets a **referrer**  share a **referral link** and **coupon code** and submit the referee's `name` and `email` via POST `addReferralData`. The API creates a `referral_details` record, sends an email to the referred person with the coupon code, and schedules follow-up emails.
...
- **Flow:**
  ...
  - `#referralCode` DOM element is set to **coupon_code**. `[data-referral='referral-link-input']` DOM element is set to `https://www.bergendebate.com?code={coupon_code}&id={memberId}`.
