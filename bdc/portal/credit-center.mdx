...
- **Credits:** Admins add credits via Retool. Retool updates `member_credit_amount`: sets `credit_balance` (increased) and pushes an entry to `credit_history` with `type` "credit"  and the `description`. The exact Retool → DB path (direct MongoDB or internal API) is outside this repo.

<Note>
  TODO:

  1. What do you mean "(or similar)" **removed text**
  2. Can we explain what type == "RemovedAdmin" means. **If we remove the credit amount from the admin, then we maintain status as a "RemovedAdmin". We are storing the amount as a negative or positive value**
</Note>
...
<Note>
  ...
  1. What is "(or equivalent)" -- in general, we should be specific, not vague.  **Removed text**
</Note>
