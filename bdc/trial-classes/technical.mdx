...
This section covers the **Acuity webhook**, how data lands in **MongoDB**, follow-on **automation and Retool**, and pointers to the **coach portal**. Parent-facing steps (free-class page, booking form, confirmation, **reschedule**, and **cancel**) are summarized in [Overview](/bdc/trial-classes/overview).

## Acuity Webhook

When a parent completes a trial class booking on the [free class page](https://www.bergendebate.com/free-class), **Acuity Scheduling** sends a webhook to **POST** `/camp/trialClassMemberData`. The Lambda **`trialClassMemberData`** processes it (create/update accounts and trial records; schedule emails; CRM lead). When a parent **reschedules** or **cancels** inside Acuity (confirmation page, email, or manage links), Acuity sends the same endpoint with **`action=rescheduled`** or **`action=cancelled`**. For **`rescheduled`**, the Lambda refetches the appointment from Acuity and **updates** the matching **`trial_class_details`** document (new date and time, **`status`: `rescheduled`**) by **`appointmentId`**. For **`cancelled`**, the Lambda updates that record’s **`status`** to **`canceled`** (same **`appointmentId`**).
...
4. **Database write**:
   - **scheduled**: Insert into `trial_class_details` with `attended: false`, `appointmentId`, `couponCode: ""`, `status: "scheduled"`
   - **rescheduled**: Update existing document by `appointmentId` (including new date/time from Acuity and **`status`: `rescheduled`**)
...
7. **For cancelled** — Update the document in `trial_class_details` where `appointmentId` matches: set **`status`** to **`canceled`**

## Database

<AccordionGroup>
  <Accordion title="trial_class_details">
...
| **status**          | **`scheduled`** after new Acuity booking; **`rescheduled`** / **`canceled`** from Acuity webhooks; **`Purchased`** when parent pays (Stripe webhook). |
...
| **email_sent**      | Count of automated emails sent.                                            |
  ...
  <Accordion title="trial_class_email_content">
...
The [Trial Class Details](https://bergendebate.retool.com/editor/41fed58a-42df-11f0-bba5-fbecf1466f1d/2025%20Phase%20B/trial_class_details/page1) page reads from `trial_class_details` (optionally joined with `members`). From this page, admins:

- **Search** and **filter** by parent, date, attendance, status
- Open parent/student details, class type, scheduled date/time, and email-sent indicators
- Open email logs (from `trial_class_email_log`)
- Add notes
- Mark a trial purchased or unpurchased (updates `status`)
