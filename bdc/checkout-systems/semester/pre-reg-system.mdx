...
Admins can view the list of bundle users on the [Bundle Management Retool page](https://bergendebate.retool.com/apps/54fa8214-ae56-11f0-a066-7f737d30b0d1/Bundle%20Management%20System). The page includes filters for **Year**, **Session**, **Parent**, and **Student**, allowing admins to quickly narrow down results.
...
Additionally, admins can add a new student to a bundle program using the **“Add Bundle Program”** button.
...
<Note>
  ...
  2. how admin can change dates pre-registration is active on the website **@SK addd description below**
</Note>

### Add / Edit Session Retool page

Admins can modify pre-registration availability by editing the session in **Manage Sessions** and updating the _Pre-Registration Start Date_ and _Pre-Registration End Date_. The frontend checks the current date against these values to enable or disable pre-registration dynamically.
...
When a user clicks “**Pre-Register**”, the system initiates the payment using the same `initializeStripePayment()` method as the standard checkout flow. Along with the usual payment data, the request includes: (1) the bundle-specific `paymentId`, which references the original payment record stored in the `payment_details` collection when the user originally pre-registered. Based on this existing payment record, the system now uses the `paymentId` to pre-register the user for the bundled program without requiring a new payment. (2) There is a flag indicating a bundle purchase `(isBundleProgram: true)`, and the selected `classUniqueId`.
...
**Upon successful payment of the bundle or brief, pre-registration will be completed by linking the classId to the corresponding paymentId.**
