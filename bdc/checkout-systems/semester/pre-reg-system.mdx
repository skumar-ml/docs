...
When a user clicks “**Pre-Register**”, the system initiates the payment using the same `initializeStripePayment()` method as the standard checkout flow. Along with the usual payment data, the request includes the bundle-specific `paymentId` ****************************************\<TODO: remind briefly what paymentId is used for..?\>****************************************\<Comment: The paymentId refers to the MongoDB ObjectId (\_id) of the original payment record stored in the payment_details collection. In the earlier transaction, the user also purchased a bundle program as part of the payment. Based on this existing payment record, the system now uses the paymentId to pre-register the user for the bundled program without requiring a new payment.\>, a flag indicating a bundle purchase `(isBundleProgram: true)`, and the selected `classUniqueId`.
