...
<Info>
  ...
  Once the payment is completed using the  Stripe checkout URL, the system updates the payment status in the `stripe_checkout_customer` collection to either Processing or Completed using the sessionId that was created during the checkout session. At the same time, a new record is inserted into the payment details collection to store the transaction information.
</Info>
