...
During the checkout session creation, we provide the success URL and cancel URL using JavaScript. After a successful payment, it redirects to the success URL with urlParam (**payment-confirmation?type=Academic&programName={label}&pType={paymentType}**)

**TODO: It will be good to explain what these URL parameters explain, or link to a section that will do it.**

- **Type(Academic | Summer)**: We have the same payment confirmation for both summer and academic registration. The type indicates which registration the payment confirmation is from. We use this for tracking purposes and analytics.
...
Before redirecting the user to the Stripe page, we set the backbuttonstate input value from 0 to 1 via code on the cart page. After the user lands to stripe page and they click on  browser back button then the user will be redirected  back to the same cart page URL. On page load, the checkout page detects backbuttonstate === "1" with valid checkout data and restores the last selected class, time slot, and bundle programs so the user can continue checkout seamlessly.

To handle the browser back button scenario, we have implemented logic that updates and detects the `backbuttonstate` input value.
