...
When the student form is submitted, the API `checkoutUrlForClasses` is called. This API generates a `checkoutId` and inserts two initial records(for the **cards** and **bank transfer**) into the `stripe_checkout_customer` collection. At this stage, the system stores the `checkoutId`, `customerId`, `paymentType`, `memberId`, `studentId`, and `parentEmail`.

<Note>
  TODO:

  1. \*\*Why two? Is one for bank transfer, and one for Credit Card. If so, make this clear\> Yes, updated the descriptions \*\*
  2. **What is customerId? Never explained, you can handle this in the TODO below.**
</Note>

### API Call and Response

After this, the user selects the class details along with optional upsells. These selections are sent to the API `updateDataToCheckoutUrl` or `checkoutUrlForUpsellProgram` (difference discussed below) with the `classId`, `bundleId`, and `briefsId`. These APIs create a Stripe checkout session for payment and update the existing records in the `stripe_checkout_customer` collection via the `checkoutId`. The updated data includes the `checkoutUrl` option for card or bank transfer. `classId`, `sessionId`, a readable label such as “Fort Lee | Level 2A | Saturday 2:15 PM – 4:15 PM”, `topicIds`, `upsellProgramIds` for bundle programs, and any applied discount amount.

<Info>
  Shubham, can we keep this sentence here?

  Once the payment is completed using the  Stripe checkout URL, the system updates the payment status in the `stripe_checkout_customer` collection to either Processing or Completed using the sessionId that was created during the payment process. At the same time, a new record is inserted into the payment details collection to store the transaction information.
</Info>
...
| ---------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| checkoutId       | Internal unique identifier for a checkout flow            | Created when `checkoutUrlForClasses` is first called                                 |
| customerId       | Stripe Customer ID representing the parent/user in Stripe | Created once on first checkout; reused for future checkouts                          |
| paymentType      | Indicates payment method (`card` or `bank_transfer`       | Two records are created initially — one per payment type                             |
| memberId         | Internal member reference                                 | Added during initial `checkoutUrlForClasses` call                                    |
| studentId        | Registered student identifier                             | Added during initial `checkoutUrlForClasses` call                                    |
| parentEmail      | Parent’s email address                                    | Added during initial `checkoutUrlForClasses` call                                    |
| checkoutUrl      | Stripe-hosted payment page URL                            | Updated when `updateDataToCheckoutUrl` or `checkoutUrlForUpsellProgram` is called    |
| classId          | Selected class                                            | Added after class selection                                                          |
| sessionId        | Stripe Checkout Session ID                                | Returned by Stripe during session creation                                           |
| Label            | Human-readable class summary                              | Created when `checkoutUrlForClasses` is first called                                 |
| topicIds         | Selected briefs program (optional)                        | Updated when `updateDataToCheckoutUrl` or `checkoutUrlForUpsellProgram` is called    |
| upsellProgramIds | Selected bundle program (optional)                        | Updated when `updateDataToCheckoutUrl` or `checkoutUrlForUpsellProgram` is called    |
| discountAmount   | Applied discount value                                    | Calculated during checkout session creation                                          |
| isClassPayment   | Flag indicating this is a class registration              | Added during initial `checkoutUrlForClasses` call                                    |
| device           | Device type (Mobile/Desktop)                              | Captured from frontend, added during initial `checkoutUrlForClasses` call            |
| deviceUserAgent  | Browser + OS fingerprint                                  | Captured from frontend, added during initial `checkoutUrlForClasses` call            |
| utmSource        | Marketing attribution source                              |                                                                                      |
| source           | Entry point (`cart_page`, etc.)                           | Passed from frontend if available, added during initial `checkoutUrlForClasses` call |
| status           | Checkout/payment state (`Pending`, `Complete`, `Failed`)  | Updated via Stripe webhook                                                           |

Depending on the purchase type, one of two APIs is called:

#### 1. Standard Checkout (`updateDataToCheckoutUrl`)

This API endpoint is used for normal class registration OR Pre-registration (e.g., pre-registered) purchases.

#### 2. Standard Checkout with any Upsell (`checkoutUrlForUpsellProgram`)

- This API endpoint is used for Pre-regstration with the bundled Program or briefs
...
> There are additional fields for **Pre-registartion** purchases:
>
> - `paymentId` – **Pre-registartion** payment identifier, 
> - `isBundleProgram` – Boolean flag, We mantain flag for the **pre-registartion**, If it's true 
...
<Note>
  ...
  **I've updated the text. We can also add this for pre-registration. Let me know If i need to add there?**
</Note>
