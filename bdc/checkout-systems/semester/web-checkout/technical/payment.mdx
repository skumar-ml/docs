...
### stripeCheckoutCustomer Collection

When does a record inserted. When is it used (for what purposes). What information does it store. When is it updated...
...
After this, the user selects the class details along with optional bundle programs, briefs, or both. These selections are sent to the API `updateDataToCheckoutUrl` with the classId, bundleId, and briefsId. This API creates a Stripe checkout session for payment and updates the existing records in the `stripe_checkout_customer` collection using the checkoutId. The updated data includes the checkoutUrl for card or bank transfer, classId, sessionId, a readable label such as “Fort Lee | Level 2A | Saturday 2:15 PM – 4:15 PM”, topicIds, upsellProgramIds for bundle programs, and any applied discount amount.
...
<Note>
  TODO: Discuss deletion logic of localStorage : **Added in the last paragraph**
</Note>

Post payment, several other operations take place, all described here **\<TODO: SK to link to doc when ready\>**.
...
## Clean Checkout Local Data

After successful payment, we delete the local data based on the checkout process, like checkoutData and referralData.