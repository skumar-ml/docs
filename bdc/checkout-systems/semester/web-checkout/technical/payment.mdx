...
When the student form is submitted, the API `checkoutUrlForClasses` is called. This API generates a `checkoutId` and inserts two initial records (one for the **card** and one for **bank transfer**) into the `stripe_checkout_customer` collection. At this stage, the system stores the `checkoutId`, `customerId`, `paymentType`, `memberId`, `studentId`, and `parentEmail`.
...
| utmSource        | Marketing attribution source                              | **TODO: this is missing** Passed from frontend if available, added during initial `checkoutUrlForClasses` call |
| source           | Entry point (`cart_page`, etc.)                           | Added during initial `checkoutUrlForClasses` call                                                              |
...
> There are additional fields for **Pre-registration** purchases:
>
...
<Note>
  ...
  **@Yogesh -- yes, let's add it there and with more details. Then, we can link to that page from here...**

  **Added in the pre-registration section**
</Note>
