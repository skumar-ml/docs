...
| memberId         | Internal member reference                                 | Added during initial `createCheckoutId` call                                     |
| studentId        | Registered student identifier                             | Added during initial `createCheckoutId` call                                     |
| parentEmail      | Parent’s email address                                    | Added during initial `createCheckoutId` call                                     |
| checkoutUrl      | Stripe-hosted payment page URL                            | Updated when `checkoutUrlForStandard` or `checkoutUrlForUpsellProgram` is called |
...
| Label            | Human-readable class summary                              | Created when `createCheckoutId` is first called                                  |
| topicIds         | Selected briefs program (optional)                        | Updated when `checkoutUrlForStandard` or `checkoutUrlForUpsellProgram` is called |
| upsellProgramIds | Selected bundle program (optional)                        | Updated when `checkoutUrlForStandard` or `checkoutUrlForUpsellProgram` is called |
| discountAmount   | Applied discount value                                    | Calculated during checkout session creation                                      |
| isClassPayment   | Flag indicating this is a class registration              | Added during initial `createCheckoutId` call                                     |
| device           | Device type (Mobile/Desktop)                              | Captured from frontend, added during initial `createCheckoutId` call             |
| deviceUserAgent  | Browser + OS fingerprint                                  | Captured from frontend, added during initial `createCheckoutId` call             |
| utmSource        | Marketing attribution source                              | Passed from frontend if available, added during initial `createCheckoutId` call  |
| source           | Entry point (`cart_page`, etc.)                           | Added during initial `createCheckoutId` call                                     |
...
#### 1. Standard Checkout (`checkoutUrlForStandard`)
