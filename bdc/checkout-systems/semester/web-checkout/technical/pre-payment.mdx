...
<Info>
  The `getUpsellProgramV2` API is triggered on page load during checkout initialization to fetch all upsell programs for the current and summer sessions. The frontend filters out `sessionId !== 2`, where `sessionId = 2` represents the summer session, to separate future bundle programs. The filtered upsell programs are then rendered in both the **banner CTA** and **bundle selection modal**.
</Info>
