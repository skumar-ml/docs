...
The `getUpsellProgramV2` API is triggered on page load during checkout initialization when the `classDetailsStripe`  **\<TODO: What is classDetailsStripe? I think it's the first time it's mentioned\>** instance is created to fetch all upsell programs for the current and summer sessions. The frontend filters out `sessionId !== 2`, where `sessionId = 2` represents the summer session, to separate future bundle programs **\<TODO: I don't understand why seperation is needed. Discuss on call\>**. The filtered upsell programs are then rendered in both the **banner CTA** and **bundle selection modal \<TODO: by which JS file?\>**.
...
**TODO: I still don't understand the Core Program Handling section. Let's discuss on call**
