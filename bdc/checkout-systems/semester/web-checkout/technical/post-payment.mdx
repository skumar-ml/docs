...
<Note>
  Initially, we identify the payment by validating the `sessionId` stored in `stripe_checkout_customer`. If it corresponds to an existing customer, we proceed with processing the payment and update both the payment status and the associated `payment_intent` object ID, which is used as the unique identifier for the transaction.
</Note>
