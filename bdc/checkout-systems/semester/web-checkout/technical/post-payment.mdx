...
_The lookup is a two-step process. First, the parent is found in the members collection using a regex pattern that matches stripeCheckout["parentEmail"] to handle single or comma-separated emails: . If a parent is found, the trial_class_details collection is queried using  — the parent's MongoDB id converted to a string. If a trial class record exists, it is updated to  using the trial class record's id. This links the payment to the trial class via the parent's email and MongoDB \_id._
...
_BDC credits are detected by checking if stripeCheckout contains "discount_amount" and it's greater than 0: if "discount_amount" in stripeCheckout and stripeCheckout["discount_amount"] \> 0. The search is performed in the member_credit_amount collection using  to find the member's credit record. If found, the credit balance is updated by subtracting the discount_amount from the current credit_balance (ensuring it doesn't go below 0), and a debit entry is added to the credit_history array with details including the amount, type "debit", description "applied during checkout", paymentId, year, and sessionId. The update is performed via , ...)._
