...
<Note>
  TODO: Can you explain the last part ("only when the main payment has a `paymentId`"). Does this code get called after inserting the main payment?
</Note>
...
<Info>
  The `bundleId` tracks the original purchase record that the bundle orginated from.
</Info>
