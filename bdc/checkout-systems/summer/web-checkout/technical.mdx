...
- Stripe events such as:
  ...
  - `charge.succeeded` / `charge.pending` / `charge.failed`
  - Async updates for **Bank Transfer**

**Responsibility:**

- Acts as the single orchestrator for all post-payment logic (same as semester checkout)
- Validates Stripe webhook signature
- Extracts `payment_intent` and `sessionId` from the event
- Finds the related record from `stripe_checkout_customer` using `sessionId`
- Normalizes Stripe events into internal payment states: **Complete**, **Processing**, **Failed**
- Routes to `updateSummerPayment()` for summer program purchases

**Key Difference from Semester:** Calls `updateSummerPayment()` instead of `updateClassPayment()` for summer program purchases.

**Implementation:** `stripePaymentListener.py`

**Reference:** See [Semester Post-Payment: stripePaymentListener](#) for webhook normalization logic.
