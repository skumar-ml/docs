...
- - Sessions are hidden only when **all** locations (e.g. Fort Lee, Glen Rock, Livingston) have `leftSpot == 0` for that session — i.e. a session is shown if at least one location has availability. **\<TODO: Are we still doing this?? We have a Livingston location now. SK: updated the code and description as well.\>**
