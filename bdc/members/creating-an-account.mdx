...
Before **any backend API is called**, the account creation flow **starts inside Memberstack**.

### What happens

- User submits **Sign Up** form (Webflow UI)
- Webflow is configured to use **Memberstack for authentication**
- Memberstack:
  - Validates email/password
  - Creates the user record in **Memberstack’s internal database**
  - Generates a unique `memberId`
  - Commits the user record **transactionally**

### How do we ensure the record exists before backend APIs run?

 **Memberstack Webhook Guarantee**

- MemberStack triggers the webhook **only after the user record is successfully created**
- The webhook is **post-insert**, not pre-insert
- If Memberstack fails to create the user, the webhook is **never fired**

This guarantees that when our backend receives the webhook:

- `memberId` already exists
- Authentication identity is finalized
- The record is durable in Memberstack

### 1.1 `POST /camp/createUserSignup `

<Note>
  TODO: Why is it /camp/? \* The /camp prefix indicates that this API belongs to the Bergen Debate Camp platform
</Note>
