...
Associations between parent and student accounts are made _at the time of registration_. When this happens, the same `familyId` is assigned to both the parent and student. If the parent does not have a `familyId`, we generate one for them, and also assign it to the student. If the parent already has a `familyId` (e.g., from a previous purchase), then that `familyId` is assigned to the student.

<Note>
  TODO:

  Are there other cases where a familyId is generated for the parent (other than during checkout)?
</Note>
...
2. Parent registers a student for a class
3. System **\<TODO which API\>** assigns a unique `familyId` to both the parent and the student

**Case 2: Existing parent adds new student**

1. Parent **\<TODO\>** <Tooltip tip="What do you mean already signed up...? I think you mean they already have a familyId generated.">(already signed up)</Tooltip> registers classes for a new student
