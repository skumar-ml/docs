...
> Back‑end storage is typically implemented with a trial‑class collection (storing appointment type, schedule, and appointments list) plus per‑student trial‑class records that keep `attended` and `notes` in sync with the above APIs.
